...
*
*               (2) The following formulas indicate the frequencies fed to the various clocks:
*
*                   f(VCO clock) = f(PLL clock input) * (PLLN/PLLM) = 8 MHz * (432/8) = 432 MHz
*
*                   f(PLL general clock output) = f(VCO clock) / PLLP = 432 MHz / 2 = 216 MHz
*
*                   f(USB OTG FS, SDMMC1/2, RNG clock output) = f(VCO clock) / PLLQ = 432 MHz / 9 = 48 MHz
*
*                   f(PLL DSI clock output) = f(VCO clock) / PLLR = 432 MHz / 7 = 61.71 MHz
*********************************************************************************************************
*/
...
void  BSP_ClkInit (void)
{
    ...
    RCC_ClkInit.AHBCLKDivider  =  RCC_SYSCLK_DIV1;              /* HCLK    = AHBxCLK = SYSCLK / AHBPRES(1) = 216 MHz    */
    RCC_ClkInit.APB1CLKDivider =  RCC_HCLK_DIV4;                /* APB1CLK = AHB1CLK / APB1DIV(4)          =  54 MHz    */
    RCC_ClkInit.APB2CLKDivider =  RCC_HCLK_DIV2;                /* APB2CLK = AHB2CLK / APB2DIV(2)          = 108 MHz    */
